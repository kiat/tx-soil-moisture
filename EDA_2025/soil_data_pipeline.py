...
# that we want to treat as contiguous missing intervals. Each entry contains a
# list of parameters and the inclusive timestamp window to append to the
# missing-summary CSV.
MANUAL_GAP_RULES = {
    5: [
        {
...
def inject_manual_gaps(station_id: int, summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for rule in rules:
        ...
        for param in rule["parameters"]:
            # Remove any existing grouped entries that overlap with this window
            overlap = (
                (df["Parameter"] == param)
