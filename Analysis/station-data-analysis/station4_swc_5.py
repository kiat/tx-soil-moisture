...
features = ['Ppt', 'T_5', 'Tair', 'RH', 'Windspeed', 'Winddirection', 'Srad']
target = ['SWC_5']
...
ar_importance_mse, ar_importance_mae, ar_importance_mae_percentage, ar_importance_r2 = custom_ar_importance(scaled_data, features, 0)  # 0 is the index of SWC_5 in the target list

# Print Custom AR results
print("\nCustom AR Results for SWC_5:")
for feature in features:
    print(f"{feature}:")
...
def display_feature_importance(importance_dict, model_name, metric_name):
    ...
    print(f"\n{model_name} Feature Importance for SWC_5 ({metric_name}):")
...
# Compare all models
if all(importance is not None for importance in [rnn_importance_mse, cnn_importance_mse, ar_importance_mse]):
    all_models_mse = pd.DataFrame({
        'RNN (MSE)': pd.Series(rnn_importance_mse),
        'CNN (MSE)': pd.Series(cnn_importance_mse),
        'Custom AR': pd.Series(ar_importance_mse)
    })
    ...
    print("\nComparison of Feature Importance Across Models for SWC_5 Prediction (Normalized and Adjusted):")
...
with open('station4_swc_5_results.csv', 'w', newline='') as csvfile:
    ...
    # Write RNN results
    for metric, importance_dict in [('MSE', rnn_importance_mse), ('MAE', rnn_importance_mae), ('MAE Percentage', rnn_importance_mae_percentage), ('R²', rnn_importance_r2)]:
        if importance_dict:
            for feature, importance in importance_dict.items():
                csvwriter.writerow(['RNN', metric, feature, f"{importance:.4f}"])
    
    # Write CNN results
    for metric, importance_dict in [('MSE', cnn_importance_mse), ('MAE', cnn_importance_mae), ('MAE Percentage', cnn_importance_mae_percentage), ('R²', cnn_importance_r2)]:
        if importance_dict:
            for feature, importance in importance_dict.items():
    ...
    for feature in features:
        csvwriter.writerow(['Custom AR', 'MSE', feature, f"{ar_importance_mse[feature]:.4f}"])
        csvwriter.writerow(['Custom AR', 'MAE', feature, f"{ar_importance_mae[feature]:.4f}"])
        csvwriter.writerow(['Custom AR', 'MAE Percentage', feature, f"{ar_importance_mae_percentage[feature]:.2f}%"])
        csvwriter.writerow(['Custom AR', 'R²', feature, f"{ar_importance_r2[feature]:.4f}"])
...
print("Results saved to station4_swc_5_results.csv")