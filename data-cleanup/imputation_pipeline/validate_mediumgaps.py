"""Validate and repair medium-gap outputs.

This script audits the SARIMAX medium-gap fills after they have been generated.
It keeps accepted fills, rejects suspicious gap segments by restoring those
timestamps to NaN, and writes separate repaired outputs so the original long
run remains intact.
"""
...
def discover_stations() -> List[str]:
    pat = re.compile(r"Station(.+)_filled_mediumgaps\.csv")
    return sorted(
        m.group(1)
        for fn in OUT_DIR.glob("Station*_filled_mediumgaps.csv")
        if (m := pat.match(fn.name))
    )


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Validate and repair medium-gap fills.")
    ...
    p.add_argument(
        "--write-repaired",
        action="store_true",
        help="Write Station*_filled_mediumgaps_repaired.csv files.",
    )
    ...
    p.add_argument("--temp-context-margin", type=float, default=10.0)
    p.add_argument("--min-training-hours", type=int, default=24)
    return p.parse_args()
...
def load_expected_segments(stations: Iterable[str], params: Iterable[str]) -> Dict[SegmentKey, int]:
    ...
    for miss_path in sorted(MISS_DIR.glob("Station*_missing_data.csv")):
        ...
        for _, row in miss.loc[mask].iterrows():
            ...
            expected[key] = int(row["Number Missing"])
    return expected
...
def load_filled_segments(stations: Iterable[str], params: Iterable[str]) -> Dict[SegmentKey, pd.DataFrame]:
    ...
    for detail_path in sorted(OUT_DIR.glob("Station*_mediumgap_fill_detail.csv")):
        station_from_name = detail_path.name[len("Station") : -len("_mediumgap_fill_detail.csv")]
        if station_from_name not in selected_stations:
            continue
        detail = pd.read_csv(detail_path, parse_dates=["Start", "End", "Timestamp"])
        if detail.empty:
            continue
        if "Station" not in detail.columns:
            continue
        ...
        for (station, param, start, end), group in detail.groupby(
            ...
            key = SegmentKey(str(station), param, start, end)
            filled[key] = group.sort_values("Timestamp").copy()
    return filled
...
def training_observed_hours(station: str, param: str, start: pd.Timestamp) -> Optional[int]:
    ...
    if param not in short.columns:
        return None
    train = short.loc[start - pd.Timedelta(days=7) : start - pd.Timedelta(hours=1), param]
    return int(train.notna().sum())
...
def context_values(
    ...
    before = short.loc[start - pd.Timedelta(days=days) : start - pd.Timedelta(hours=1), param]
    after = short.loc[end + pd.Timedelta(hours=1) : end + pd.Timedelta(days=days), param]
...
def segment_metrics(
    ...
    if key.parameter in medium.columns:
        x = medium.loc[key.start : key.end, key.parameter]
    ...
    if key.parameter in short.columns:
        left_ts = key.start - pd.Timedelta(hours=1)
        right_ts = key.end + pd.Timedelta(hours=1)
        left = short.loc[left_ts, key.parameter] if left_ts in short.index else pd.NA
        right = short.loc[right_ts, key.parameter] if right_ts in short.index else pd.NA
        ...
        if key.parameter.startswith("T_"):
            ctx = context_values(short, key.parameter, key.start, key.end)
            if len(ctx) >= args.min_training_hours:
...
def build_summary(args: argparse.Namespace) -> Tuple[pd.DataFrame, Dict[SegmentKey, pd.DataFrame]]:
    ...
    all_keys = sorted(
        set(expected) | set(filled),
        key=lambda k: (k.station, k.parameter, k.start, k.end),
    )

    medium_cache: Dict[str, pd.DataFrame] = {}
    short_cache: Dict[str, pd.DataFrame] = {}
    ...
    for key in all_keys:
        ...
        if key in filled:
            ...
            observed = training_observed_hours(key.station, key.parameter, key.start)
            row.update(
                {
                    "Status": "skipped",
                    "Reason": (
                        "too_few_training_hours"
                        if observed is not None and observed < args.min_training_hours
                        else "not_in_fill_detail"
                    ),
                    "Training Observed Hours": observed,
                }
            )
        rows.append(row)
...
def write_repaired_outputs(
    ...
    for station in stations:
        ...
        repaired.to_csv(OUT_DIR / f"Station{station}_filled_mediumgaps_repaired.csv")

        details = accepted_by_station.get(station, [])
        if details:
            pd.concat(details, ignore_index=True).to_csv(
                OUT_DIR / f"Station{station}_mediumgap_fill_detail_repaired.csv",
                index=False,
            )
...
def write_summaries(summary: pd.DataFrame, report_dir: Path) -> None:
    ...
    summary_path = report_dir / "mediumgaps_validation_summary.csv"
    rejected_path = report_dir / "mediumgaps_rejected_segments.csv"
    station_path = report_dir / "mediumgaps_validation_station_summary.csv"
...
def main() -> None:
    ...
    print("Medium-gap validation complete.")
    print("Segments:", counts)
    print(f"Summary: {args.report_dir / 'mediumgaps_validation_summary.csv'}")
    print(f"Rejected: {args.report_dir / 'mediumgaps_rejected_segments.csv'}")
    print(f"Station summary: {args.report_dir / 'mediumgaps_validation_station_summary.csv'}")
    if args.write_repaired:
        print("Repaired station files written to output/*_filled_mediumgaps_repaired.csv")
    else:
