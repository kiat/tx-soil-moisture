...
df1 = pd.read_csv('satellite/all_merged_data/Station1_AMSR_SMAP_Merged.csv', parse_dates=['Date'])
print(f"df1 shape: {df1.shape}")
print(f"df1 date range: {df1['Date'].min()} to {df1['Date'].max()}")

# Read the second CSV file
df2 = pd.read_csv('datasets/Merged-Cleaned-Simulated-Data/Station1-Simulated-cleaned-merged-data.csv', parse_dates=['Date'])
...
amsr_df = pd.read_csv('satellite/AMSR_data_csv/AMSR_station_1_data.csv', parse_dates=['Date'])
smap_df = pd.read_csv('satellite/satellite_data_csv/Station1_Satellite.csv', parse_dates=['Date'])
...
merged_df.to_csv('satellite/all_merged_data/Station1_AMSR_SMAP_Merged.csv', index=False, date_format='%Y-%m-%d')
