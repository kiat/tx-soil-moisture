...
def load_station_data(config):
    """Loads station data from the specified path in config."""
    
    # Ensure path is absolute regardless of where main.py is run
    project_root = Path(__file__).resolve().parent.parent.parent  # Moves up one level from `src/`
    data_folder = project_root / config["paths"]["data_folder"]
    
    print(f"Looking for data in: {data_folder}")  # Debugging print
