    ...
    def plot(self, model_name, config="", model=None, plot_cols=['Predicted_Rain_Next_Hour_1h', 'Predicted_Rain_Next_Hour_3h', 'Predicted_Rain_Next_Hour_6h', 'Predicted_Rain_Next_Hour_12h', 'Predicted_Rain_Next_Hour_24h'], max_subplots=3):
        ...
        for n in range(max_n):
            ...
            for i, plot_col in enumerate(plot_cols):
                ...
                plt.scatter(self.label_indices, labels[n, :, label_col_index], label=f'Label ({plot_col})', edgecolors='k', s=32)

                if model is not None:
                    predictions = model(inputs)
                    plt.scatter(self.label_indices, predictions[n, :, i], label=f'Prediction ({plot_col})', edgecolors='k', s=32)
