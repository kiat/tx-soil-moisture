###  demo.ipynb

- Showcases the use of all scripts, with the command-line usage for each.

# Scripts

Run these from inside the `scripts/` folder — the modules import one another by name. Every script except `soil_or_met.py` and `read_data.py` also has a CLI (shown below); the single-file tools take `<input.dat> <output.csv>`.

Prewash pipeline order: **dup_cleaner → treat_subhourly_data → time_cleaner → treat_wrong_data**.

---
## 1. soil_or_met.py  *(import only)*

- Tools to determine whether a data file pertains to soil measurements, weather (met) measurements, or is unknown.

---
## 2. read_data.py  *(import only)*

- contains: check_for_NaT, read_soil, read_met, and file_to_indexed_df.

- **file_to_indexed_df** calls the rest after you provide **a file path** to the raw .dat file and specify whether it is a 'soil' or 'met' file (see soil_or_met.py to automate this). Returns a DataFrame with the timestamps as the index.

- **read_soil** — assuming a soil file like CB01.dat, the method will:
    1. read the CSV skipping the first 5 lines, parsing the measurement columns as native numerics and keeping `Flag` as a string
    2. convert the `Date` column to a datetime (format `%Y-%m-%d %H:%M:%S`, else coerced to `NaT`) and set it as the index
    3. warn if any `NaT` ended up in the index
    4. sort the index stably (ascending)
    5. coerce any stray non-numeric measurement values to `NaN` (`Flag` stays a string)
    6. return the DatetimeIndexed DataFrame

- **read_met** — assuming a met file like CB01_met.dat, the method will:
    1. read the CSV using line 6 as the header, skipping the two units / measurement-type rows and dropping `RECORD` at read time
    2. standardize the column names (`TIMESTAMP`→`Date`, `Rain_mm_Tot`→`Ppt`, …)
    3. convert the `Date` column to a datetime and set it as the index
    4. warn if any `NaT` ended up in the index
    5. sort the index stably (ascending)
    6. coerce any stray non-numeric values to `NaN`
    7. return the DatetimeIndexed DataFrame

---
## 3. dup_cleaner.py

- Expects a DataFrame with a timestamp index and a "Flag" column.

- Treats the three cases of duplicate timestamps (all keep the first occurrence):
    1. same timestamp, measurements, and flag
    2. same timestamp, measurements, but different flag
    3. same timestamp, different measurements

- Returns a DataFrame with the duplicates resolved and a DatetimeIndex.

- **CLI:** `python dup_cleaner.py <input.dat> <output.csv>`

---
## 4. treat_subhourly_data.py

- Condenses sub-hourly readings (timestamps with non-zero minutes) onto the hourly grid: sub-hourly precipitation (`Ppt`) is summed into the next whole hour, every other measurement keeps the value already on the whole hour, and the sub-hourly rows are dropped. Run after dup_cleaner and before time_cleaner.

- **CLI:** `python treat_subhourly_data.py <input.dat> <output.csv>`

---
## 5. time_cleaner.py

- Expects a df with a unique, whole-hour DatetimeIndex.

- Returns a DatetimeIndexed df with the missing hourly timestamps filled in with `NaN` measurements.

- **CLI:** `python time_cleaner.py <input.dat> <output.csv>`

---
## 6. treat_wrong_data.py

- Using hardcoded columns and acceptable ranges, replaces all values deemed impossible with `NA` (the `Flag` column is left untouched).

- **CLI:** `python treat_wrong_data.py <input.dat> <output.csv>`

---
## 7. gap_report.py

- Tallies data gaps by duration (`<24h`, `1-7d`, `7-30d`, `>30d`) for both the timestamp index (missing hours) and each column (consecutive-`NaN` runs); prints and returns the table. Takes in prewashed data.

- **CLI:** `python gap_report.py <input.csv> [output.csv]`  (output CSV optional)

---
## 8. get_data_dict.py

- The full data ingest (read + prewash + clean) pipeline over a folder of raw files. Prewashing applies dup_cleaner → treat_subhourly_data → time_cleaner → treat_wrong_data to each station; cleaning (impute) is not yet implemented.

- **CLI:** `python get_data_dict.py <input_folder> --prewash --download [--prewash-folder DIR]`

---
## 9. synthetic_gap.py  *(import only)*

- **synthetic_gap(df, length, column=None, start=None, seed=None, context=None)** — punches one artificial NaN gap into a prewashed station DataFrame and returns `(gapped, answer)`: the frame with the hole, and the ground-truth answer key. The gap is only placed where every target column has real data. `length`/`context` accept row counts or time spans like `"36h"`.

---
## 10. gap_sampler.py  *(import only, no CLI yet)*

- **sample_gaps(data_dir, n_gaps, seed, column, lengths, strata, rain_frac, context, pool_max, met)** — draws a random sample of synthetic gaps across all stations (the random sampler from `shortGap_imputation.ipynb` / `mediumGap_imputation.ipynb` as a reusable function). `met=False` samples the soil stations, `met=True` the `*_met` stations; by default any measurement column can be gapped (`Flag` never is, and it stays a string throughout). Station files are found by the prewash naming convention (`_met` suffix). `lengths` gives the exact gap lengths to draw (None -> lengths matching the durations of real gaps, durations only — every sampled gap is synthetic); `strata=True` splits `n_gaps` evenly across the distinct lengths, `False` draws them at random; `rain_frac` controls the share of rain-in-gap windows at every length. Samples are strictly synthetic — windows touching a real gap in any column are rejected, so the punched gap is the only missing data in each returned frame. Returns a list of sample dicts (`station, column, length_h, start, rain_in_gap, gapped, answer`).

- **Pipeline, function by function:** a call starts at `sample_gaps(...)`, which validates `data_dir` and seeds the RNG. `_station_files(data_dir, met)` scans the folder by the prewash naming convention and returns a dict of station name → csv path (soil or `_met` files). Stations are read on demand by `load()`, which returns each station's Date-indexed DataFrame (Flag kept as a string) and caches it. When `lengths=None`, `_gap_lengths(df, columns, max_rows)` — run over `_default_columns(df)`, every column but Flag — returns the length in rows of each real gap shorter than `pool_max`, and these pool across stations into `length_pool`. `_plan_slots(length_pool, n_gaps, strata, rain_frac, rng)` turns that pool into the plan: a list of `n_gaps` `(hours, want_rain)` pairs, split evenly across distinct lengths if `strata=True`, drawn randomly otherwise, with rain requirements balanced at every length. The draw loop then fills each slot: it picks a random station and column (rainy slots anchor on an hour from `rain_rows()`), and `synthetic_gap(df, ...)` returns `(gapped, answer)` — the frame with an artificial NaN hole and the ground truth for it; any window holding a real NaN or touching a record edge is rejected and redrawn. Accepted draws become sample dicts (`station, column, length_h, start, rain_in_gap, gapped, answer`); `_print_tally(...)` prints the length × rain composition, and `sample_gaps` returns the list, ready for `score_method(impute, samples)` to turn into a per-gap DataFrame of rmse/mae/bias. When `lengths=None` the empirical SWC gap-length pool is measured straight from the data — no `gap_durations/` folder needed; `pool_max` caps the admitted gap lengths (default `"24h"`).

---
## 11. score_method.py  *(import only, no CLI yet)*

- **score_method(impute, samples, bins=None)** — scores an `impute(gapped, column) -> Series` gap-filling function on every synthetic gap from `gap_sampler.sample_gaps`. Returns one row per gap (station, column, length_h, rain_in_gap, rmse, mae, bias, length_bin). By default the tables group by exact gap length, matching `gap_sampler`'s draws; pass `bins` edges to group lengths into ranges instead. Pass a `name` to also print the method's report (overall RMSE / MAE / bias plus a length x rain summary table).
